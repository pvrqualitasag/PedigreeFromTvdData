...
set_field_na <- function(ptbl_pedigree, 
                         ...
                         ps_field_idx,
                         pn_pk_col_idx = getTvdIdColsDsch()$TierIdCol){
  quo_field <- dplyr::enquo(ps_field_idx)
  vec_row_idx <- sapply(pvec_rec_pk, function(x) which(ptbl_pedigree[,pn_pk_col_idx] == x), USE.NAMES = FALSE)
  vec_org_val <- ptbl_pedigree %>% dplyr::select(!!quo_field)
  vec_org_val[vec_row_idx] <- NA
  tbl_pedigree_vna <- ptbl_pedigree %>% dplyr::mutate(!!quo_field := vec_org_val)
  return(tbl_pedigree_vna)
}
...
pvec_rec_pk = vec_rec_pk
# ps_field_idx = 
# quo_field <- dplyr::enquo(V5)
pn_pk_col_idx = getTvdIdColsDsch()$TierIdCol
...
(vec_org_val <- ptbl_pedigree %>% dplyr::select(V5))


```

```{r}
(vec_org_val[vec_row_idx,])
...
Testing the same operation as above using the function


```{r}
tbl_pedigree_vna_func <- set_field_na(tbl_pedigree, vec_rec_pk, V5)
```
