...
```{r PackageSetup, eval=TRUE, include=FALSE}
...
## Method selection
In a compromise between speed of data input and ease of use of the resulting data object during consistency checks, we are currently looking at `readr::read_fwf()`. 
...
The timing indicates how long it takes to do the check of the TVD-Ids. When we are interested in the amount of memory taken by the pedigree and by the check, we follow the outline given in [Advanced R](http://adv-r.had.co.nz/memory.html). The size in memory of the complete pedigree is
...
The change in memory usage during the check can be measured using
...
```{r}
# checks for mothers and fathers
all(tbl_ped_checked[[2]][!is.na(tbl_ped_checked[[2]])] == tbl_ped[[2]][!is.na(tbl_ped[[2]])])
all(tbl_ped_checked[[9]][!is.na(tbl_ped_checked[[9]])] == tbl_ped[[9]][!is.na(tbl_ped[[9]])])
```
