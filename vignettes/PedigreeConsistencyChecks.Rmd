...
```{r}
# checks for mothers
(l_check_result_mothers <- all_parent_id_consistent(p_tbl_ped = tbl_ped, pn_parent_col = getTvdIdCols()$MutterIdCol))
```
The same check is also done for the fathers.

```{r}
# checks for fathers
(l_check_result_fathers <- all_parent_id_consistent(p_tbl_ped = tbl_ped, pn_parent_col = getTvdIdCols()$VaterIdCol))
```
