...
```{r}
tbl_transform_ped %>% inner_join(tbl_fail_mother_age, by = c("V12" = "Animal")) %>% select(V5,V11,V12,V16)
tbl_transform_ped %>% inner_join(tbl_fail_mother_age, by = c("V12" = "Parent")) %>% select(V5,V11,V12,V16)
```

Comparing the transformed pedigree records to the original records shows that birthdates are invalidated.

```{r}
tbl_ped_uni_id %>% inner_join(tbl_fail_mother_age, by = c("V12" = "Animal")) %>% select(V5,V11,V12,V16)
tbl_ped_uni_id %>% inner_join(tbl_fail_mother_age, by = c("V12" = "Parent")) %>% select(V5,V11,V12,V16)
```
